...
**Patrón obligatorio de arranque (las tres tareas de interfaz).** Nunca registrar
trabajo de inicio con `window.addEventListener("pywebviewready", fn)` a secas:
pywebview dispara ese evento al inyectar su API, y si eso ocurre antes de que el
script se ejecute, el oyente no salta jamás. Usar siempre:
