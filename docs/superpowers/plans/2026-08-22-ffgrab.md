...
- Dependencias de producción: `yt-dlp`, `pywebview` y `truststore` (este último solo para validar TLS contra el almacén de certificados de Windows; ver `app.py:_usar_certificados_del_sistema`). Sin framework web, sin bundler, sin dependencias de frontend. La interfaz es HTML, CSS y JavaScript sin compilar.
...
**Patrón obligatorio de arranque (las tres tareas de interfaz).** Comprobar
`window.pywebview.api` NO basta: ese objeto existe antes de que sus métodos
estén enganchados, así que una llamada inmediata devuelve el error "puente no
disponible" y el trabajo de arranque se pierde en silencio. La condición
correcta es que el **método** sea invocable. Centralizado en `api.js`:
...
Las tareas 10, 11 y 12 registran su trabajo de arranque con
`window.FFGrab.alEstarListo(fn)`, nunca con `addEventListener("pywebviewready", fn)`
a secas.
